...
from app.services.feature_metadata import (
    build_feature_lineage_entry,
    derive_feature_training_metadata,
    hydrate_feature_pipeline_metadata,
    resolve_feature_pipeline_metadata,
    resolve_feature_pipeline_input_columns,
)
from app.services.feature_templates import BUILTIN_FEATURE_TEMPLATES
...
def run_feature_pipeline_job(job_id: int, pipeline_id: int) -> None:
    with SessionLocal() as db:
        ...
        try:
            ...
            frame, feature_lineage = _apply_steps(frame, pipeline.steps, collect_lineage=True)
            ...
            (
                ...
                pipeline.business_context_columns,
                pipeline.analysis_retained_columns,
                pipeline.feature_lineage,
            ) = derive_feature_training_metadata(frame, input_columns, feature_lineage=feature_lineage)
            db.add(pipeline)
...
def list_feature_pipelines(db: Session, project_id: int, dataset_version_id: int | None = None) -> list[FeaturePipeline]:
    ...
    for pipeline in pipelines:
        hydrate_feature_pipeline_metadata(db, pipeline)
    return pipelines
...
def _apply_steps(
    ...
    *,
    ...
    for raw_step in steps:
        ...
        current = _apply_single_step(current, normalized_step)
        if collect_lineage:
            for column in current.columns:
                if column in before_columns:
                    continue
                feature_lineage[column] = build_feature_lineage_entry(
                    column=column,
                    step=normalized_step,
                )
    current = current.reset_index(drop=True)
    if collect_lineage:
        return current, feature_lineage
    return current
